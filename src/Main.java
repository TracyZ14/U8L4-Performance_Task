    ...
    public static int findHighestHourglass(int[][] matrix)
    {
        ...
        for(int row = 1; row < (matrix.length - 1); row++)
        {
            for(int column = 1; column < (matrix[row].length - 1); column++)
            {
                ...
                int topMiddle = matrix[row - 1][column];
                int topRight = matrix[row - 1][column + 1];
                int middle = matrix[row][column];
                int bottomLeft = matrix[row + 1][column - 1];
                int bottomMiddle = matrix[row + 1][column];
                int bottomRight = matrix[row + 1][column + 1];
                int sum = topLeft + topMiddle + topRight + middle + bottomLeft + bottomMiddle + bottomRight;
                if(sum > greatestSum)
                {
                    greatestSum = sum;
                }
